...
// POST /api/auth/register  (first-user setup only; authenticated invites use /api/users)
auth.post("/register", async (c) => {
	...
	// Check if any users exist — first user gets auto-created, subsequent users are created from /api/users.
	const count = await c.env.DB.prepare(
		"SELECT COUNT(*) as cnt FROM users",
	).first<{ cnt: number }>();
	if (count && count.cnt > 0) {
		return c.json(
			{ error: "Registration is closed. Ask an admin to invite you." },
			403,
		);
	}
